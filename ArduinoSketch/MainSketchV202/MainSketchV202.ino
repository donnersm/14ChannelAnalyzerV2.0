...
//   Version 2.02  Build in functionality to add logo ledstrip, no more need for extra arduino tiny. 
...
bool Showmode = false;
char barRed = 0, barGreen = 255, barBlue = 0;
char topRed = 255, topGreen = 255, topBlue = 0;
...
// the next part of variable and definitions is for the logo
...
Adafruit_NeoPixel LogoStrip = Adafruit_NeoPixel(LOGOLED_COUNT, LOGO_PIN, NEO_GRB + NEO_KHZ800); // declare Logo Strip
int looptimer=0; // to count the loops, not to update the logo every single loop
#define LogoBrightness 50  // Set BRIGHTNESS of logo
long firstPixelHueStripLogo = 0;
/*
...
dbgprint ("Configuring datalines for MSGEQ7");
  ...
  LogoStrip.begin();           // INITIALIZE NeoPixel Logo strip object (REQUIRED)
  LogoStrip.show();            // Turn OFF all pixels ASAP
  LogoStrip.setBrightness(LogoBrightness); // set Brightness of logo
...
void loop()
{ 
 // update routine logo color 
 looptimer++;
if(looptimer==5){   
firstPixelHueStripLogo += 256;


    for(int i=0; i<LogoStrip.numPixels(); i++) { // For each pixel in strip...
      int pixelHue2 = firstPixelHueStripLogo + (i * 65536L / (LogoStrip.numPixels() *10));
      LogoStrip.setPixelColor(i, LogoStrip.gamma32(LogoStrip.ColorHSV(pixelHue2)));
    }
    LogoStrip.show(); // Update strip with new contents
looptimer=0;
} 
// update routine logo color end
...
{
  DEBUG = 0;  //this is needed to fully use the dbgprint function
  dbgprint("                                                                                      ");
